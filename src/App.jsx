...
const EMPTY_FORM = {
  ...
  vencimiento: "", precio: "", documento: "", usuarioTv: "", claveTv: "", dispositivos: [], zona: "", referidoPor: "", notas: "",
};
...
export default function App() {
  ...
  const abrirEditar = useCallback((c) => {
    setForm({ ...c, dispositivos: getDispositivos(c) }); setClienteActivo(c); setModal("editar");
  }, []);
  ...
  return (
    <div style={{ fontFamily:"Inter,sans-serif", background:"#0F1117", minHeight:"100vh", color:"#E8EAF0", maxWidth:480, margin:"0 auto" }}>
      ...
      <div style={{ display:"flex", background:"#13162A", borderBottom:"1px solid #1E2340", padding:"3px 6px" }}>
        {[["cobros","🔔 Cobros"],["dashboard","📊 Panel"],["clientes","👥 Clientes"],["pagos","💰 Pagos"],["graficas","📈 Graficas"]].map(([v,l]) => (
      ...
      {modal==="detalle" && clienteActivo && (
        <ModalBg onClose={()=>setModal(null)}>
            ...
            {getDispositivos(clienteActivo).length > 0 && (
              <>
      ...
      {(modal==="nuevo"||modal==="editar") && (
        <ModalBg onClose={()=>setModal(null)}>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
            <label style={{ ...lbl, marginBottom:0 }}>📺 Dispositivos / Perfiles</label>
            <button
              onClick={()=>setForm(prev=>({ ...prev, dispositivos:[...(prev.dispositivos||getDispositivos(prev)), { nombre:"", usuario:"", clave:"" }] }))}
              style={{ background:"#1E2B6B", border:"none", color:"#60A5FA", borderRadius:8, padding:"4px 10px", fontSize:11, fontWeight:600, cursor:"pointer" }}
            >+ Agregar</button>
          </div>
          ...
          {(!form.dispositivos || form.dispositivos.length===0) && getDispositivos(form).length===0 && (
            <div style={{ fontSize:11, color:"#4B5563", textAlign:"center", padding:"8px 0 12px" }}>Sin dispositivos. Toca "+ Agregar" para anadir uno.</div>
          )}
          <div style={{ height:8 }}/>
          <div style={{ display:"flex",gap:8 }}>
      ...
      {modal==="confirmarEliminar" && clienteAEliminar && (
        <ModalBg onClose={()=>{ setModal("detalle"); setClienteAEliminar(null); }}>
          ...
          <Btn onClick={()=>{ setModal("detalle"); setClienteAEliminar(null); }}>Cancelar</Btn>
        </ModalBg>
      )}
