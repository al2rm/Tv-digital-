...
export default function App() {
  ...
  const [subVistaCobros, setSubVistaCobros] = useState("vencidos");
...
<body>
  ...
  function reactivarCliente(c) {
    const updated = { ...c, categoria:"normal" };
    const newList = clientes.map(x => x.id===c.id ? updated : x);
    saveClientes(newList);
    syncCliente(updated);
  ...
  const cobrosPendientes = useMemo(() => {
    return clientes
      .filter(c => c.categoria!=="suspendido" && calcularEstado(c.vencimiento, config.diasAlerta).dias <= 0)
      .sort((a,b) => a.vencimiento > b.vencimiento ? 1 : -1);
  }, [clientes, config.diasAlerta]);

  const clientesSuspendidos = useMemo(() => {
    return clientes
      .filter(c => c.categoria==="suspendido")
      .sort((a,b) => a.vencimiento > b.vencimiento ? 1 : -1);
  }, [clientes]);
      ...
      <div style={{ padding:12, paddingBottom:80 }}>
        ...
        {vista==="cobros" && <>
          ...
          <div style={{ display:"flex", gap:6, marginBottom:14 }}>
            <button onClick={()=>setSubVistaCobros("vencidos")}
              style={{ flex:1, border:"none", background:subVistaCobros==="vencidos"?"#1E2B6B":"#1A1D2E", color:subVistaCobros==="vencidos"?"#60A5FA":"#6B7280", borderRadius:10, padding:"9px 0", fontSize:12, fontWeight:subVistaCobros==="vencidos"?700:500, cursor:"pointer" }}
            >🔴 Vencidos {cobrosPendientes.length>0 && "("+cobrosPendientes.length+")"}</button>
            <button onClick={()=>setSubVistaCobros("suspendidos")}
              style={{ flex:1, border:"none", background:subVistaCobros==="suspendidos"?"#37415144":"#1A1D2E", color:subVistaCobros==="suspendidos"?"#E8EAF0":"#6B7280", borderRadius:10, padding:"9px 0", fontSize:12, fontWeight:subVistaCobros==="suspendidos"?700:500, cursor:"pointer" }}
            >⛔ Suspendidos {clientesSuspendidos.length>0 && "("+clientesSuspendidos.length+")"}</button>
          </div>

          {subVistaCobros==="vencidos" && <>
          ...
          {subVistaCobros==="suspendidos" && <>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"baseline", marginBottom:4 }}>
            <div style={secTitle}>⛔ Clientes suspendidos</div>
            <span style={{ fontSize:11, color:"#6B7280" }}>{clientesSuspendidos.length} cliente{clientesSuspendidos.length!==1?"s":""}</span>
          </div>
          <div style={{ fontSize:11, color:"#6B7280", marginBottom:12 }}>Estos clientes no aparecen en la lista de cobros diarios</div>
          {clientesSuspendidos.length===0 && (
            <div style={{ textAlign:"center", padding:"50px 20px", color:"#4B5563" }}>
              <div style={{ fontSize:50, marginBottom:12 }}>✅</div>
              <div style={{ fontSize:15, fontWeight:600, color:"#6B7280" }}>Sin clientes suspendidos</div>
            </div>
          )}
          {clientesSuspendidos.map(c => {
            const est = calcularEstado(c.vencimiento, config.diasAlerta);
            return (
              <div key={c.id} style={{ ...clientCard, border:"1px solid #37415155", opacity:0.85 }} onClick={()=>abrirDetalle(c)}>
                <div style={avatar("#374151")}>{c.nombre[0].toUpperCase()}</div>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontWeight:600, fontSize:14 }}>{c.nombre}</div>
                  <div style={{ fontSize:11, color:"#6B7280" }}>{planLabel(c.plan)} · Vencio: {formatFecha(c.vencimiento)}</div>
                </div>
                <button
                  onClick={e=>{e.stopPropagation();reactivarCliente(c);}}
                  style={{ background:"#10B98122", border:"none", color:"#10B981", borderRadius:9, padding:"6px 10px", fontSize:10, fontWeight:600, cursor:"pointer" }}
                >Reactivar</button>
              </div>
            );
          })}
