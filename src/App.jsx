  ...
  return (
    <div style={{ fontFamily:"Inter,sans-serif", background:"#0F1117", minHeight:"100vh", color:"#E8EAF0", maxWidth:480, margin:"0 auto" }}>
      ...
      <div style={{ display:"flex", background:"#13162A", borderBottom:"1px solid #1E2340", padding:"3px 6px", overflowX:"auto" }}>
        {[["cobros","🔔 Cobros"],["dashboard","📊 Panel"],["clientes","👥 Clientes"],["usuariostv","🎫 TV"],["pagos","💰 Pagos"],["graficas","📈 Graficas"]].map(([v,l]) => (
      ...
      <div style={{ padding:12, paddingBottom:80 }}>
                ...
                {expandida && (
                  <div style={{ padding:"0 14px 14px" }}>
                    ...
                    {perfilesClientes.map((cliente, idx) => {
                      ...
                      if (!cliente) {
                        return (
                          <button key={numPerfil} onClick={()=>abrirNuevoParaPerfil(cuenta.id, numPerfil)}
                            style={{ width:"100%", background:"#0F1117", border:"1px dashed #2A2D45", borderRadius:10, padding:"12px 12px", marginBottom:7, cursor:"pointer", color:"#4B5563", fontSize:12, textAlign:"left", display:"flex", alignItems:"center", gap:8 }}
                          ><span style={{ fontSize:16 }}>➕</span> Agregar cliente al Perfil {numPerfil}</button>
                        );
                      }
        ...
        {vista==="graficas" && <>
          <div style={{ ...card, background:"linear-gradient(135deg,#1a1200,#2a1f0022)", border:"1px solid #FBBF2433" }}>
            <div style={{ fontSize:11, fontWeight:600, color:"#FBBF24", marginBottom:8, textTransform:"uppercase" }}>📄 Reporte mensual</div>
            ...
            <div style={{ fontSize:10, color:"#6B7280", marginTop:6 }}>Se abre en una ventana nueva, lista para guardar como PDF</div>
          </div>
